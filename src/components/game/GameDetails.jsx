    ...
    <GameDetailsWrapper>
      ...
      <div className='details-grid d-grid'>
        <div className='details-left img-fit-cover'>
          <LazyLoadImage src = { gameData?.background_image } alt = { gameData?.name } />
        </div>

        <div className='details-right'>
          <h4 className='details-right-title fw-7 text-green mb-3'>Game<span className = 'text-white'>Details</span></h4>
          <div className='para-text' dangerouslySetInnerHTML={{__html:(gameData?.description)?.split('.').splice(0, 2).join('.') + "."}}></div>
      ...
      <Tabs>
        ...
        <TabPanel>
          ...
          <div className="para-text" dangerouslySetInnerHTML={{ __html:gameData?.description}}>
          </div>
        </TabPanel>
        <TabPanel>
